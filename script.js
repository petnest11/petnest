    ...
    card.innerHTML = `
      ...
      pet.id
        ...
        <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
          ...
          <button class="btn primary adopt-btn" data-id="${
            pet.id
          }">Adopt</button>
        </div>
      </div>
...
// ----- EVENT HANDLERS FOR FAVORITES + MODAL BUTTONS -----
...
function attachAdoptEvents() {
  document.querySelectorAll(".adopt-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      alert(
        `✅ Adoption Request Sent!\n\nWe will send all the adoption information about ${pet.name} to your mail shortly.`
      );

      // OPTIONAL: Auto-fill contact form message and scroll
      const messageBox = document.getElementById("message");
      if (messageBox) {
        messageBox.value = `I am interested in adopting ${pet.name}. Please send me the details.`;
...
function openPetModal(id) {
  ...
  modalBodyEl.innerHTML = `
    ...
    pet.name
  }" style="width:100%;border-radius:18px;margin:0.8rem 0;max-height:260px;object-fit:cover;" />
    <p>${pet.description}</p>
